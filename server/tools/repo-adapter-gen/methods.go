...
func (r renderer) generateMethods(methods map[string]methodSig, root string) ([]byte, []harvestTarget, int, error) {
	...
	for _, name := range names {
		...
		case isGen:
			if len(cands) == 0 {
				return nil, nil, 0, fmt.Errorf("method %q is in methods_gen.go but no longer fits a generatable shape; hand-write it and delete it from methods_gen.go", name)
			}
			if hasHand {
			...
			if c == nil {
				return nil, nil, 0, fmt.Errorf("method %q in methods_gen.go matches no generated shape; hand-write it and delete it from methods_gen.go", name)
			}
			body.WriteString("\n")
	...
	if err != nil {
		return nil, nil, 0, fmt.Errorf("format methods source: %w\n%s", err, b.String())
	}
	return formatted, harvest, handCount, nil
}
