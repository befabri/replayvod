...
func (a *SQLiteAdapter) SetJobExecution(ctx context.Context, jobID, executionID string, acceptsMetadata bool) error {
	return executionAffected(a.queries.SetJobExecution(ctx, sqlitegen.SetJobExecutionParams{ID: jobID, ExecutionID: executionID, AcceptsMetadata: boolToInt64(acceptsMetadata)}))
}
func (a *SQLiteAdapter) StopJobMetadata(ctx context.Context, jobID, executionID string) error {
	return executionAffected(a.queries.StopJobMetadata(ctx, sqlitegen.StopJobMetadataParams{ID: jobID, ExecutionID: executionID}))
}

func (a *SQLiteAdapter) CheckpointAttempt(ctx context.Context, jobID, executionID string, state json.RawMessage) error {
	return executionAffected(a.queries.CheckpointAttempt(ctx, sqlitegen.CheckpointAttemptParams{ID: jobID, ExecutionID: executionID, ResumeState: string(state)}))
}
func (a *SQLiteAdapter) ListRecoveryJobs(ctx context.Context, afterID string, limit int) ([]repository.Job, error) {
	rows, err := a.queries.ListRecoveryJobs(ctx, sqlitegen.ListRecoveryJobsParams{ID: afterID, Limit: int64(limit)})
	if err != nil {
		return nil, err
	}
...
func (a *SQLiteAdapter) ListQueuedArchiveJobs(ctx context.Context, after time.Time, afterID int64, limit int) ([]repository.ArchiveQueueCandidate, error) {
	rows, err := a.queries.ListQueuedArchiveJobs(ctx, sqlitegen.ListQueuedArchiveJobsParams{AfterStart: sqliteTime(after), AfterID: afterID, BatchLimit: int64(limit)})
	if err != nil {
		return nil, err
	}
...
func (a *SQLiteAdapter) ListStoppedJobs(ctx context.Context, afterID string, limit int) ([]repository.Job, error) {
	rows, err := a.queries.ListStoppedJobs(ctx, sqlitegen.ListStoppedJobsParams{ID: afterID, Limit: int64(limit)})
	if err != nil {
		return nil, err
	}
	out := make([]repository.Job, len(rows))
