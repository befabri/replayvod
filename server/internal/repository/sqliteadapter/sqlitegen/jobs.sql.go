...
SELECT id, video_id, broadcaster_id, status, started_at, finished_at, error, resume_state, created_at, updated_at, attempt FROM jobs WHERE id = ?
`
...
SELECT id, video_id, broadcaster_id, status, started_at, finished_at, error, resume_state, created_at, updated_at, attempt FROM jobs WHERE video_id = ? ORDER BY created_at DESC LIMIT 1
`
