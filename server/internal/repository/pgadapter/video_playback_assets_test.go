...
func seedDonePlaybackVideo(t *testing.T, ctx context.Context, a *PGAdapter, jobID, filename, broadcasterID string, parts int) int64 {
	t.Helper()
...
func upsertReadyAsset(t *testing.T, ctx context.Context, a *PGAdapter, videoID int64, name string, lastAccessed time.Time) {
	t.Helper()
	mime := "video/mp4"
...
func TestListReadyVideoPlaybackAssetsLRUOrder(t *testing.T) {
	...
	upsertReadyAsset(t, ctx, a, v2, "rec-lru-2-playback.mp4", now.Add(-3*time.Hour))
