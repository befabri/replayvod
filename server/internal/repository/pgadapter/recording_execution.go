...
func (a *PGAdapter) SetJobExecution(ctx context.Context, jobID, executionID string, acceptsMetadata bool) error {
	return executionAffected(a.queries.SetJobExecution(ctx, pggen.SetJobExecutionParams{ID: jobID, ExecutionID: executionID, AcceptsMetadata: acceptsMetadata}))
}
func (a *PGAdapter) StopJobMetadata(ctx context.Context, jobID, executionID string) error {
	return executionAffected(a.queries.StopJobMetadata(ctx, pggen.StopJobMetadataParams{ID: jobID, ExecutionID: executionID}))
}

func (a *PGAdapter) CheckpointAttempt(ctx context.Context, jobID, executionID string, state json.RawMessage) error {
	return executionAffected(a.queries.CheckpointAttempt(ctx, pggen.CheckpointAttemptParams{ID: jobID, ExecutionID: executionID, ResumeState: state}))
}
func (a *PGAdapter) ListRecoveryJobs(ctx context.Context, afterID string, limit int) ([]repository.Job, error) {
	rows, err := a.queries.ListRecoveryJobs(ctx, pggen.ListRecoveryJobsParams{ID: afterID, Limit: int32(limit)})
	if err != nil {
		return nil, err
	}
...
func (a *PGAdapter) ListQueuedArchiveJobs(ctx context.Context, after time.Time, afterID int64, limit int) ([]repository.ArchiveQueueCandidate, error) {
	rows, err := a.queries.ListQueuedArchiveJobs(ctx, pggen.ListQueuedArchiveJobsParams{AfterStart: after, AfterID: afterID, BatchLimit: int32(limit)})
	if err != nil {
		return nil, err
	}
...
func (a *PGAdapter) ListStoppedJobs(ctx context.Context, afterID string, limit int) ([]repository.Job, error) {
	rows, err := a.queries.ListStoppedJobs(ctx, pggen.ListStoppedJobsParams{ID: afterID, Limit: int32(limit)})
	if err != nil {
		return nil, err
	}
	out := make([]repository.Job, len(rows))
