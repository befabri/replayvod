// Package system owns owner-level admin business logic: user/role
// management, whitelist CRUD, fetch log reads, event log reads, event
// log search. Each service method carries the invariant the transport
// layer can't enforce on its own (e.g. "owner can't demote self").
package system
