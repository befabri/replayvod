...
func (s *Service) UpdateUserRole(ctx context.Context, input UpdateUserRoleInput) (UserInfo, error) {
	...
	u, err := s.svc.UpdateUserRole(ctx, caller.ID, input.UserID, input.Role)
	if err != nil {
		if errors.Is(err, systemservice.ErrCannotDemoteSelf) {
		...
		s.log.Error("update user role", "user_id", input.UserID, "error", err)
