...
// + whitelist + role + user upsert flow lives in authservice.
type Handler struct {
	cfg        *config.Config
...
// NewHandler creates a new auth handler wired to a domain authservice.
func NewHandler(cfg *config.Config, tc *twitch.Client, sm *session.Manager, svc *authservice.Service, log *slog.Logger) *Handler {
	return &Handler{
		cfg:        cfg,
		twitch:     tc,
		sessionMgr: sm,
...
// authservice.HandleOAuthCallback: validates state + PKCE cookies,
// hands the code + verifier off to the service, and turns the result
// into a cookie + redirect. State tampering (constant-time compare),
// missing cookies, and provider-returned errors all short-circuit
// before the service gets invoked.
func (h *Handler) handleCallback(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		var denied *authservice.ErrLoginDenied
