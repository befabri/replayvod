...
// TestMatch_TableDriven covers every branch of the matcher's decision
// tree. Each case is named after the behavior under test so a
// regression in the matcher surfaces in the test name, not just a
// boolean mismatch.
func TestMatch_TableDriven(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			filters:  Filters{Categories: []repository.Category{{ID: "509658"}, {ID: "33214"}}},
		...
		},
		{
			...
			schedule: repository.DownloadSchedule{HasCategories: true},
			filters:  Filters{Categories: []repository.Category{{ID: "509658"}}},
		...
		},
		{
			...
			schedule: repository.DownloadSchedule{HasCategories: true},
			filters:  Filters{Categories: nil},
			signals:  StreamSignals{CategoryIDs: []string{"509658"}},
			want:     false,
		},
		{
		...
		},
		{
			...
			filters:  Filters{Tags: []repository.Tag{{ID: 1}, {ID: 2}}},
		...
		},
		{
			name:     "tag mismatch",
			schedule: repository.DownloadSchedule{HasTags: true},
			filters:  Filters{Tags: []repository.Tag{{ID: 1}}},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := Match(&tc.schedule, tc.filters, tc.signals)
			if got != tc.want {
...
// TestMatch_NilSchedule_NeverMatches guards against a nil-pointer panic
// on the webhook hot path. The dispatcher loops over
...
func TestMatch_NilSchedule_NeverMatches(t *testing.T) {
	if Match(nil, Filters{}, StreamSignals{}) {
		t.Fatal("nil schedule must never match")
	}
}

// TestMatch_EmptySignals_NoOpsFilteredSchedules pins the Phase 5
...
func TestMatch_EmptySignals_NoOpsFilteredSchedules(t *testing.T) {
	minViewers := int64(10)
	cases := []struct {
		name     string
		schedule repository.DownloadSchedule
		...
		},
		{
			...
			schedule: repository.DownloadSchedule{HasCategories: true},
			filters:  Filters{Categories: []repository.Category{{ID: "cat"}}},
		},
		{
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if Match(&tc.schedule, tc.filters, StreamSignals{}) {
