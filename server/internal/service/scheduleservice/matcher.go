// Package scheduleservice houses the pure schedule-matching function and
// the webhook-driven auto-download trigger.
//
// MatchSchedule is the heart of the auto-record feature: given a user's
// download policy and a live stream's signals (viewer count, categories,
// tags), decide whether to kick off a download. It must be cheap and
// allocation-free on the hot path — it runs on every stream.online
// webhook for every active schedule on the affected channel.
...
// ScheduleFilters enumerates the precomputed link rows for a schedule,
// avoiding a repeat query in the tight matching loop on the webhook path.
type ScheduleFilters struct {
	Categories []repository.Category
	Tags       []repository.Tag
}

// MatchSchedule returns true when a schedule should trigger a download
// for the observed stream signals.
//
// Semantics:
...
func MatchSchedule(schedule *repository.DownloadSchedule, filters ScheduleFilters, signals StreamSignals) bool {
