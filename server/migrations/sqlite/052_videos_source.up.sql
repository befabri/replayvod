-- Archived VODs share the videos table with live recordings. source tells them
-- apart, twitch_video_id is the Twitch VOD id and broadcast_at the date the
-- stream originally aired. Archives retry transient failures: jobs.attempt
-- numbers the attempts of one recording (jobs accumulate per attempt) and
-- videos.next_retry_at is set on a failed archive that will be requeued. One
-- open row per VOD: not removed, and either not failed or waiting for a retry.
...
ALTER TABLE videos ADD COLUMN broadcast_at TEXT;
ALTER TABLE videos ADD COLUMN next_retry_at TEXT;
