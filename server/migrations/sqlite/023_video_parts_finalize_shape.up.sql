-- See postgres/023_video_parts_finalize_shape.up.sql for design
-- comments. SQLite can't ALTER COLUMN DROP NOT NULL in place, and
-- ADD COLUMN rejects the `(datetime('now'))` default, so we rebuild
-- the table. Backfill updated_at from created_at for existing rows.
