...
ALTER TABLE videos
    ADD COLUMN trigger_schedule_id BIGINT REFERENCES download_schedules(id) ON DELETE SET NULL,
    ADD COLUMN retention_source_schedule_id BIGINT REFERENCES download_schedules(id) ON DELETE SET NULL,
    ADD COLUMN retention_window_hours INTEGER,
    ADD CONSTRAINT chk_videos_retention_window_hours
