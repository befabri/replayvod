...
UPDATE jobs SET execution_id = @execution_id, accepts_metadata = @accepts_metadata,
...
UPDATE jobs SET accepts_metadata = 0 WHERE id = @job_id AND execution_id = @execution_id AND status = 'RUNNING';
...
UPDATE jobs SET resume_state = @state, updated_at = datetime('now')
WHERE jobs.id = @job_id AND jobs.execution_id = @execution_id AND jobs.status = 'RUNNING'
...
WHERE jobs.id > @after_id AND (jobs.status = 'RUNNING' OR (jobs.status = 'PENDING' AND videos.source = 'live'))
  AND videos.status IN ('PENDING', 'RUNNING') AND videos.deleted_at IS NULL
ORDER BY jobs.id LIMIT @limit;
...
AND (videos.start_download_at > sqlc.arg(after) OR (videos.start_download_at = sqlc.arg(after) AND videos.id > CAST(sqlc.arg(after_id) AS BIGINT)))
ORDER BY videos.start_download_at, videos.id LIMIT sqlc.arg(limit);

-- name: ListStoppedJobs :many
SELECT jobs.* FROM jobs JOIN videos ON videos.id = jobs.video_id AND videos.job_id = jobs.id
WHERE jobs.id > @after_id AND jobs.stop_requested = 1 AND jobs.status IN ('PENDING', 'RUNNING')
  AND videos.status IN ('PENDING', 'RUNNING') AND videos.deleted_at IS NULL
ORDER BY jobs.id LIMIT @limit;
