...
UPDATE recording_intents SET status='waiting',wait_until=CAST(sqlc.arg(wait_until_value) AS TEXT) WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(current_job_id) AND status='active' AND stop_requested=0;

-- name: ActivateRecordingIntent :execrows
UPDATE recording_intents SET status='active',current_job_id=sqlc.arg(next_job_id),last_stream_id=sqlc.arg(last_stream_id),wait_until=NULL WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(previous_job_id) AND status='waiting' AND stop_requested=0 AND wait_until >= CAST(sqlc.arg(observed_at_value) AS TEXT);
