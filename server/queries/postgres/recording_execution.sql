-- name: GetVideoForUpdate :one
SELECT * FROM videos WHERE id = $1 FOR UPDATE;

-- name: SetJobExecution :execrows
UPDATE jobs SET execution_id = @execution_id, accepts_metadata = @accepts_metadata,
    status = 'RUNNING', started_at = COALESCE(started_at, NOW()), updated_at = NOW()
WHERE id = @job_id AND status IN ('PENDING', 'RUNNING');

-- name: StopJobMetadata :execrows
UPDATE jobs SET accepts_metadata = FALSE
WHERE id = @job_id AND execution_id = @execution_id AND status = 'RUNNING';

-- name: RequestJobStop :exec
UPDATE jobs SET stop_requested = TRUE, accepts_metadata = FALSE, updated_at = NOW()
WHERE id = $1 AND status IN ('PENDING', 'RUNNING');

-- name: CheckpointAttempt :execrows
UPDATE jobs SET resume_state = @state, updated_at = NOW()
WHERE jobs.id = @job_id AND jobs.execution_id = @execution_id AND jobs.status = 'RUNNING'
  AND EXISTS (SELECT 1 FROM videos WHERE videos.id = jobs.video_id
      AND videos.job_id = jobs.id AND videos.status = 'RUNNING' AND videos.deleted_at IS NULL);

-- name: ListRecoveryJobs :many
SELECT jobs.* FROM jobs JOIN videos ON videos.id = jobs.video_id AND videos.job_id = jobs.id
WHERE jobs.id > @after_id AND (jobs.status = 'RUNNING' OR (jobs.status = 'PENDING' AND videos.source = 'live'))
  AND videos.status IN ('PENDING', 'RUNNING') AND videos.deleted_at IS NULL
ORDER BY jobs.id LIMIT sqlc.arg('limit');

-- name: ListQueuedArchiveJobs :many
SELECT jobs.id AS job_id, videos.id AS video_id, videos.start_download_at AS queued_at FROM jobs JOIN videos ON videos.id = jobs.video_id AND videos.job_id = jobs.id
WHERE jobs.status = 'PENDING' AND videos.status = 'PENDING' AND videos.source = 'vod' AND videos.deleted_at IS NULL
AND (videos.start_download_at > sqlc.arg(after) OR (videos.start_download_at = sqlc.arg(after) AND videos.id > CAST(sqlc.arg(after_id) AS BIGINT)))
ORDER BY videos.start_download_at, videos.id LIMIT sqlc.arg('limit');

-- name: ListStoppedJobs :many
SELECT jobs.* FROM jobs JOIN videos ON videos.id = jobs.video_id AND videos.job_id = jobs.id
WHERE jobs.id > @after_id AND jobs.stop_requested AND jobs.status IN ('PENDING', 'RUNNING')
  AND videos.status IN ('PENDING', 'RUNNING') AND videos.deleted_at IS NULL
ORDER BY jobs.id LIMIT sqlc.arg('limit');
