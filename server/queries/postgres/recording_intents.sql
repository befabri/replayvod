...
UPDATE recording_intents SET status='waiting',wait_until=sqlc.arg(until) WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(job_id) AND status='active' AND stop_requested=FALSE;

-- name: ActivateRecordingIntent :execrows
UPDATE recording_intents SET status='active',current_job_id=sqlc.arg(next_job_id),last_stream_id=sqlc.arg(stream_id),wait_until=NULL WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(previous_job_id) AND status='waiting' AND stop_requested=FALSE AND wait_until >= sqlc.arg(observed_at);
