...
SELECT * FROM recording_intents WHERE id = sqlc.arg(id) FOR UPDATE;
...
SELECT recording_intents.* FROM recording_intents WHERE recording_intents.id>sqlc.arg(after_id) AND (recording_intents.status IN ('active','waiting') OR EXISTS (SELECT 1 FROM recording_intent_videos r JOIN videos v ON v.id=r.video_id JOIN jobs j ON j.id=v.job_id WHERE r.intent_id=recording_intents.id AND j.status IN ('PENDING','RUNNING'))) ORDER BY recording_intents.id LIMIT sqlc.arg(batch_limit);

-- name: SetRecordingIntentWaiting :execrows
UPDATE recording_intents SET status='waiting',wait_until=sqlc.arg(wait_until) WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(current_job_id) AND status='active' AND stop_requested=FALSE;

-- name: ActivateRecordingIntent :execrows
UPDATE recording_intents SET status='active',current_job_id=sqlc.arg(next_job_id),last_stream_id=sqlc.arg(last_stream_id),wait_until=NULL WHERE id=sqlc.arg(id) AND current_job_id=sqlc.arg(previous_job_id) AND status='waiting' AND stop_requested=FALSE AND wait_until >= sqlc.arg(observed_at);
...
UPDATE recording_intents SET stop_requested=TRUE WHERE id=sqlc.arg(id);
...
WHERE r.intent_id=sqlc.arg(intent_id) AND jobs.id>sqlc.arg(after_id) AND jobs.status IN ('RUNNING','PENDING') AND videos.deleted_at IS NULL ORDER BY jobs.id LIMIT sqlc.arg(batch_limit);
