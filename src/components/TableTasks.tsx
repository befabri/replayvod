      ...
      <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
          {items.map((item, index) => (
            <tr key={index}>
              <td className="px-6 py-4 whitespace-nowrap">{t(item.name)}</td>
              <td className="px-6 py-4 whitespace-nowrap">{formatInterval(item.interval)}</td>
              <td className="px-6 py-4 whitespace-nowrap">
                {formatIntervalPast(Date.now() - new Date(item.lastExecution).getTime())}
              </td>
              <td className="px-6 py-4 whitespace-nowrap">{formatInterval(item.lastDuration)}</td>
              <td className="px-6 py-4 whitespace-nowrap">
