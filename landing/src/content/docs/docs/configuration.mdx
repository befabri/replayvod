---
title: Configuration
description: How ReplayVOD is configured — the two files, what each one is for, and precedence.
---

import { Aside, Card, CardGrid, FileTree } from '@astrojs/starlight/components';

ReplayVOD has two configuration files.

<CardGrid>
  <Card title="server/.env">
    Credentials, database selection, public URLs, local paths, Connect relay URLs. Treat as secret.
  </Card>
  <Card title="server/config.toml">
    Operational behaviour: download policy, scheduler intervals, logging, storage backend, health endpoint, title-tracking mode.
  </Card>
</CardGrid>

<FileTree>

- replayvod/
  - server/
    - .env             secrets, paths, network
    - .env.example     committed template — every supported var is listed and commented here
    - config.toml      tuning
  - docker-compose.yml
  - data/

</FileTree>

## Precedence

`server/.env` always wins. The Go process reads the env file first, then the TOML file. For live/title behavior, a complete `SERVER_MODE` env config also wins over app-managed server settings and skips onboarding. If `SERVER_MODE` is empty, the owner dashboard stores server mode setup in the database, and mode or URL changes are applied on the next server start.

The Docker Compose file reads `server/.env` directly. A few `PUBLIC_*` variables are Compose-only — they shape what gets passed into the container without changing the stored values.

## Required values

```text
TWITCH_CLIENT_ID=...
TWITCH_SECRET=...
SESSION_SECRET=...
OWNER_TWITCH_ID=...  # optional; empty = first login becomes owner
```

The installer generates `SESSION_SECRET`; for manual installs, generate it with `openssl rand -hex 32`. `OWNER_TWITCH_ID` is optional and only preassigns a numeric Twitch user ID as owner; when empty, the first successful login becomes owner. The EventSub HMAC secret is optional: leave it unset and the server generates and stores one in the database on first run, or set `HMAC_SECRET` to seed a specific value the first time.

## Public URLs

For normal deployments, set `PUBLIC_BASE_URL` and let the server derive the
OAuth callback, frontend redirect URL, and trusted browser origin:

```text
PUBLIC_BASE_URL=https://replayvod.example
```

If the dashboard is hosted on a different origin from the API, add it with
`TRUSTED_ORIGINS`:

```text
TRUSTED_ORIGINS=https://dashboard.example
```

Direct EventSub env setup is optional; leaving `SERVER_MODE` empty lets the
owner configure live/title behavior in the dashboard.

## By area

<CardGrid>
  <Card title="Storage">
    Local filesystem or S3-compatible — see [Storage](/docs/storage/).
  </Card>
  <Card title="Recording">
    Quality, codecs, retries, gap tolerance — see [Recording](/docs/recording/).
  </Card>
  <Card title="Schedules & title tracking">
    Recording rules and server modes — see [Schedules](/docs/schedules/).
  </Card>
  <Card title="EventSub">
    Direct EventSub setup — see [EventSub](/docs/eventsub/).
  </Card>
  <Card title="Connect relay">
    Pointing the recorder at a hosted relay — see [Connect relay](/docs/connect/).
  </Card>
</CardGrid>

For a flat lookup of every variable or TOML key, see the reference:

- [Environment variables](/docs/reference/env/)
- [config.toml](/docs/reference/config-toml/)

<Aside type="caution" title="Don't lose server/.env">
  `server/.env` contains the secrets used to encrypt sessions and verify webhook HMACs. Losing it invalidates sessions and can break EventSub verification.
</Aside>
