...
<Aside type="caution" title="Don't lose server/.env">
  Sessions can be recovered by signing in again. The EventSub HMAC secret lives in the database (`server_settings`), so it travels with a database backup and existing subscriptions keep verifying after a restore. Restart the recorder after restoring; the boot reconcile recreates whatever is missing.
</Aside>
