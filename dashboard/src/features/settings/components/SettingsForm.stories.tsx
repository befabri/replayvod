import { expect } from "storybook/test";
import preview from "#.storybook/preview";
import i18n from "@/i18n";
import { USER_SETTINGS } from "@/test/fixtures";
import { layoutMismatches } from "@/test/layout";
import { SettingsForm, SettingsFormSkeleton } from "./SettingsForm";

const meta = preview.meta({
	title: "Features/Settings/SettingsForm",
	component: SettingsForm,
	args: { data: USER_SETTINGS },
	argTypes: { data: { control: false } },
	parameters: { layout: "padded" },
});

export const Default = meta.story();

export const Loading = meta.story({
	render: () => <SettingsFormSkeleton />,
	play: async ({ canvas }) => {
		await expect(
			canvas.getByRole("status", { name: i18n.t("common.loading") }),
		).toBeVisible();
	},
});

// The skeleton draws the form's own labels and hints and holds each field's
// place, so nothing moves when the settings arrive.
export const SkeletonMatchesCard = meta.story({
	render: (args) => (
		<div className="grid gap-8">
			<div data-testid="skeleton">
				<SettingsFormSkeleton />
			</div>
			<div data-testid="loaded">
				<SettingsForm {...args} />
			</div>
		</div>
	),
	play: async ({ canvas }) => {
		await expect(
			layoutMismatches(
				canvas.getByTestId("skeleton"),
				canvas.getByTestId("loaded"),
			),
		).toEqual([]);
	},
});
